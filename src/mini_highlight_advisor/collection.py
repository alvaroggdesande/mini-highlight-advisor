...
def load(path: Path | None = None, catalog: list[PaintColor] | None = None) -> set[str]:
    path = Path(path) if path is not None else COLLECTION_PATH
    if not path.exists():
        return set()
...
def save(owned: set[str], path: Path | None = None) -> None:
    path = Path(path) if path is not None else COLLECTION_PATH
